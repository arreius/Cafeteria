...
controller.guardarUsuarios = (req, res) => {
  ...
  req.getConnection((err, connection) => {
    connection.query(
      "INSERT INTO `usuarios` (`nombrePersonal`, `nombreUsuario`, `contraseña`, `direccion`, `tipoUsuario`, `telefono`, `email`) VALUES(?, ?, ?,?,?,?,?)",
      [
        data.nombrePersonal,
        data.nombreUsuario,
        data.contraseña,
...
controller.guardarPlatillo = (req, res) => {
  ...
  req.getConnection((err, connection) => {
    connection.query(
      "INSERT INTO `producto` (`nombreProducto`, `descripcion`, `precio`, `stock`, `idCategoria`) VALUES(?, ?, ?,?,?)",
      [
        data.nombreProducto,
        data.descripcion,
        data.precio,
...
controller.guardarContacto = (req, res) => {
  ...
  (usr = usuario),
    ...
    req.getConnection((err, connection) => {
      connection.query(
        "INSERT INTO `contacto` (`nombreContacto`,`numeroContacto`, `mailContacto`, `mensajeContacto`, `contactado`) VALUES(?,?, ?, ?,?)",
        [data.nombre, data.telefono, data.mail, data.mensaje, 0],
        (err, customer) => {
          res.render(
            "index",
            ...
            {
              contactado:
                "Registrado de manera adecuada, ahora puede ingresar!",
...
controller.carrito = (req, res) => {
  ...
  req.getConnection((err, connection) => {
    connection.query(
      "INSERT INTO `factura` (`idUsuario`,`fechaCompra`) VALUES(?,SYSDATE())",
      [usuario]
    );
      ...
      for (let i = 0; i < dataParseada.length; i++) {
        connection.query(
          ...
          [
            ...
            idFactura,
            dataParseada[i].id,
            dataParseada[i].cantidad,
            dataParseada[i].precio,
          ]
        );
      }
    });
...
function crearPDF(data, user, idFac) {
  ...
  <h3> <center>Factura No.` +
    idFac +
    `</center></h3>
  <h3> <center>ID de Usuario: ` +
    user +
    `</center></h3>
        <hr>
        <div class="my-5">
    ...
    `
            ...
            <tfoot>
            <td>Total</td><td></td><td></td><td></td><td> Q.` +
    costoTotal +
    `</td>
            </tfoot>
            </table>
  ...
  pdf
    .create(content, formato)
    .toFile("./Factura-No." + idFac + ".pdf", function (err, res) {
      if (err) {
        console.log(err);
      } else {
        siguiente();
...
controller.updatePendiente = (req, res) => {
  ...
  (usr = usuario),
    ...
    req.getConnection((err, conn) => {
      conn.query(
        "UPDATE `contacto` SET `contactado`=? WHERE `contacto`.`idContacto`=?",
        [1, id],
        (err, rows) => {
...
controller.verFactura = (req, res) => {
  ...
  (usr = usuario),
    ...
    req.getConnection((err, conn) => {
      conn.query(
        "SELECT * FROM detalle WHERE numFactura=? ORDER BY idDetalle",
        [id],
        (err, rows) => {
...
controller.updateProducto = (req, res) => {
  ...
  (usr = usuario),
    ...
    req.getConnection((err, conn) => {
      conn.query(
        "UPDATE `producto` SET `nombreProducto`=?,`descripcion`=?,`precio`=?,`stock`=?,`idCategoria`=? WHERE `producto`.`idProducto` =?",
        [
          productoActualizado.nombreProducto,
...
controller.updateUsuario = (req, res) => {
  ...
  (usr = usuario),
    ...
    req.getConnection((err, conn) => {
      conn.query(
        "UPDATE `usuarios` SET `nombrePersonal`=?,`nombreUsuario`=?,`contraseña`=?,`direccion`=?,`tipoUsuario`=?,`telefono`=?,`email`=? WHERE `usuarios`.`idUsuario`= ?",
        [
          usuarioActualizado.nombrePersonal,
