...
function addRow(id, nombre, precio) {
  ...
  if (indexProducto === -1) {
    ...
    arregloCarrito[indexProducto].cantidad =
      arregloCarrito[indexProducto].cantidad + 1;
  }
  ...
  for (let i = 0; i < arregloCarrito.length; i++) {
    ...
    $("table tbody").append(
      "<tr><td>" +
        arregloCarrito[i].id +
        "</td> <td>" +
        arregloCarrito[i].nombre +
        "</td><td>" +
        arregloCarrito[i].cantidad +
        "</td> <td><button class='btn btn-info btn-sm' onclick='sumarCantidad(" +
        i +
        ");'> + </button><button class='btn btn-danger btn-sm' onclick='restarCantidad(" +
        i +
        ");'>-</button> </td>     <td>Q." +
        num +
        "</td> </tr>"
    ...
    costoTotal =
      costoTotal + arregloCarrito[i].precio * arregloCarrito[i].cantidad;
  }
  if (arregloCarrito.length != 0) {
    ...
    $("table tfoot").append(
      "<td>Total</td><td></td><td> " +
        ...
        "</td><td> <button class='btn btn-info' onclick='comprar();'>Comprar todo</button>    <button type='button' class='btn btn-warning' onclick='borrarTodo();'>Borrar Todo</button> </td> <td> Q." +
        costoTotal +
        "</td>"
    );
